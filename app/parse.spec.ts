...
const examplePayload = `0:[["children","(main)","children","__PAGE__",["__PAGE__",{}],"$L1",[[],["$L2",["$","meta",null,{"name":"next-size-adjust"}]]]]]
...
describe("getType", () => {
  it("should find no type on an object", () => {
    expect(getType("{}")).toStrictEqual(undefined);
  });

  it("should find no type on an array", () => {
    expect(getType("[]")).toStrictEqual(undefined);
  });

  it("should find find an import type on an object", () => {
    expect(getType("I{}")).toStrictEqual("I");
  });

  it("should find find an import type on an array", () => {
    expect(getType("I[]")).toStrictEqual("I");
  });

  it("should find find an multi-character type", () => {
    expect(getType("HZ[]")).toStrictEqual("HZ");
  });
...
describe("extract", () => {
  it("should extract signifiers and line types", () => {
    const lines = ["0:I{}", "1:{}", "a:[[{}]]", "7:[]", "5:I[]"];
    expect(extract(lines)).toStrictEqual([
      {
        signifier: "0",
        rawType: "I",
        rawJson: "{}",
